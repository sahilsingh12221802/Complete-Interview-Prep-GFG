...
{
    public:
    void dfs(Node* node, vector<Node*>& path, vector<bool>& visited, int k, unordered_set<Node*>& result){
        if(!node) return;
        path.push_back(node);
        visited.push_back(false);
        if(!node->left && !node->right){
            if(path.size() > k){
                Node* ancestor = path[path.size() - k - 1];
                result.insert(ancestor);
            }
        }
        dfs(node->left, path, visited, k, result);
        dfs(node->right, path, visited, k, result);
        path.pop_back();
        visited.pop_back();
    }
    ...
    int printKDistantfromLeaf(Node* root, int k)
    {
    	...
    	dfs(root, path, visited, k, result);
    	return result.size();
    }
};
